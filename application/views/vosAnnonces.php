...
<?php
echo '<h1>Annonces partenaire</h1>';
	echo '<table border=0>';
	
foreach ($model1 as $ligne): 

	echo '<tr><td>'.$ligne['sport'].'</td><td>'.$ligne['lieu_rdv'].'</td>
	<td>'.$ligne['date_dispo'].'</td><td>'.$ligne['description'].'</td><td>'.$ligne['titre_annonce'].'</td><td>'.$ligne['dureeDistance'].'</td></tr>';

endforeach;

echo '</table>';

echo '<h1>Annonces matériel</h1>';
	echo '<table border=0>';
